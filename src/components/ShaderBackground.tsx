...
const ShaderBackground = ({ className = '' }: ShaderBackgroundProps) => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      const float offsetSpeed = 1.28 * overallSpeed;
      ...
      void main() {
        ...
        space.y += random(space.x * warpFrequency + iTime * warpSpeed) * warpAmplitude * (0.45 + horizontalFade);
        space.x += random(space.y * warpFrequency + iTime * warpSpeed + 2.0) * warpAmplitude * horizontalFade;

        vec4 lines = vec4(0.0);

        vec4 bgColor1 = vec4(0.02, 0.02, 0.08, 1.0);
        vec4 bgColor2 = vec4(0.16, 0.06, 0.28, 1.0);
